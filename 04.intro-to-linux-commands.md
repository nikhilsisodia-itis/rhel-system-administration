...
- Use `f` to go forward one page and `b` to go back one page.
- Use `k` to scroll up one line and `j` to scroll down one line.
- Use `d` to scroll down half a page and `u` to scroll up half a page.
- You can also use the arrow keys to navigate through the manual page.
