...
- `cat /proc/meminfo`: Displays detailed memory information from the `/proc` filesystem
- `vmstat 1`: Provides a real-time view of system performance, including memory usage, every second.
- `top` or `htop`: Interactive tools that display real-time system information, including memory usage by processes.
